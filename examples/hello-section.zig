...
const symtab = elf.headers.symtab;
...
pub fn main() anyerror!void {
    ...
    const symb_text = Symbol(.Elf64) {
        .name = "",
        .value = program_seg.vaddr,
        // .size = program_seg.data.len,
        .size = 0,
        .type_ = symtab.STT.Section,
        .bind = symtab.STB.Local,
        .visbility = symtab.STV.Default,
        .shndx = (4) + 0,
    };

    try elf_file.segments.append(program_seg);

    try elf_file.sections.append(text_sec);

    try elf_file.symbols.append(symb_text);

    const out_file_name = "hello_elf_section";
